...
to the ClawdeckAgent and used throughout the browser automation system.
"""
...
class TestShowBrowserFlag:
    ...
    def test_agent_initialization_show_browser_false(self):
        ...
        with patch('clawdeck.agent.BrowserAuth'):
    ...
    def test_browser_auth_initialization_headless_mode(self):
        ...
        with patch('clawdeck.agent.AnthropicModel'):
            with patch('clawdeck.agent.BrowserAuth') as mock_browser_auth:
                # Set API key in environment to avoid validation errors
                os.environ['ANTHROPIC_API_KEY'] = 'test-key'
    ...
    def test_environment_variable_browser_show_true(self):
        """Test CLAWDECK_BROWSER_SHOW environment variable enables browser visibility"""
        # Test that environment variable enables show_browser
        original_env = os.environ.get('CLAWDECK_BROWSER_SHOW')
        try:
            os.environ['CLAWDECK_BROWSER_SHOW'] = 'true'
            ...
            if original_env is None:
                os.environ.pop('CLAWDECK_BROWSER_SHOW', None)
            else:
                os.environ['CLAWDECK_BROWSER_SHOW'] = original_env

    def test_environment_variable_browser_show_false(self):
        """Test CLAWDECK_BROWSER_SHOW environment variable defaults to headless"""
        # Test that environment variable defaults to headless mode
        original_env = os.environ.get('CLAWDECK_BROWSER_SHOW')
        try:
            os.environ['CLAWDECK_BROWSER_SHOW'] = 'false'
    ...
    def test_cli_flag_overrides_environment_variable(self):
        ...
        original_env = os.environ.get('CLAWDECK_BROWSER_SHOW')
        try:
            os.environ['CLAWDECK_BROWSER_SHOW'] = 'false'  # Env says headless
