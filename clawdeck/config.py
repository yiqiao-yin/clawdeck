"""Configuration management for Clawdeck CLI.

This module handles loading and merging configuration from:
1. User config: ~/.clawdeck/config.yaml
2. Project config: .clawdeck.yaml (in current directory)
3. Default values
...
def load_user_config() -> Dict[str, Any]:
    """
    Load user configuration from ~/.clawdeck/config.yaml.
...
def load_project_config() -> Dict[str, Any]:
    """
    Load project configuration from .clawdeck.yaml in current directory.
...
def merge_configs(user_config: Dict[str, Any], project_config: Dict[str, Any]) -> ClawdeckConfig:
    """
    Merge user and project configurations with default values.

    Configuration precedence (highest to lowest):
    1. Project config (.clawdeck.yaml)
    ...
    Returns:
        ClawdeckConfig object with merged configuration
    """
...
def load_env_config() -> Dict[str, Any]:
    ...
    Supports browser automation optimization settings with CLAWDECK_ prefix.
...
def load_config() -> ClawdeckConfig:
    """
    Load and merge all configuration sources.

    This is the main entry point for configuration loading.
    Configuration precedence (highest to lowest):
    1. Environment variables (CLAWDECK_*)
    2. Project config (.clawdeck.yaml)
    3. User config (~/.clawdeck/config.yaml)
    4. Default values

    Returns:
        ClawdeckConfig object with merged configuration from all sources
    """
...
def create_default_user_config() -> bool:
    """
    Create a default user configuration file at ~/.clawdeck/config.yaml.
    ...
    default_config = """# Clawdeck CLI User Configuration
...
def create_default_project_config() -> bool:
    """
    Create a default project configuration file at .clawdeck.yaml in current directory.

    Returns:
        True if file was created, False if it already exists or there was an error
    """
    config_path = Path.cwd() / ".clawdeck.yaml"

    # Don't overwrite existing config
    if config_path.exists():
        return False

    default_config = """# Clawdeck CLI Project Configuration
# Location: .clawdeck.yaml (in project root)
