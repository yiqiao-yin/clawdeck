"""Clawdeck Agent - AI coding assistant using pydantic_ai"""
...
from .config import ClawdeckConfig
...
class ClawdeckAgent:
    """
    Clawdeck AI coding assistant agent.
    ...
    def __init__(
        ...
        """
        Initialize the Clawdeck Agent with Anthropic, AWS Bedrock, Google Gemini, or OpenAI.
        ...
        self.cache_dir = Path.home() / ".clawdeck" / "cache"
        ...
        if config and config.browser_use_cache_enabled and HAS_CRAWL4AI:
            cache_dir = Path.home() / ".clawdeck" / "cache" / "fetched_sites"
        ...
        debug_mode = os.getenv('CLAWDECK_BROWSER_DEBUG', 'false').lower() == 'true'
    ...
    def _get_system_prompt(self) -> str:
        ...
        base_prompt = """You are Clawdeck, an intelligent AI coding assistant. Your role is to help users with:
...
- Examples:
  - ✅ "Login to https://clawdecksearch.com with my credentials"
...
**Usage Example:**
```
User: Login to https://clawdecksearch.com with eagle0504 and password123
You: login_to_website(
    url="https://clawdecksearch.com/login",
    username="eagle0504",
    password="password123"
)

After successful login:
User: Fetch my profile from https://clawdecksearch.com/profile
You: fetch_website("https://clawdecksearch.com/profile")  # Automatically uses saved session!
```
...
**Storage Location:**
- Credentials: `~/.clawdeck/credentials/vault.enc` (encrypted)
- Sessions: `~/.clawdeck/sessions/*.session.json`
- Audit log: `~/.clawdeck/logs/auth_audit.log`
...
{preview}"""
    ...
    async def execute_command(
        ...
        if sys.stdin.isatty() and os.getenv('CLAWDECK_SKIP_CONFIRM') != '1':
...
This Space was created with Clawdeck-CLI.
...
'''
    ...
    async def fetch_website(
        ...
        **Authenticated Browsing (Phase 4.3):**
        ...
        Examples:
            ...
            - "Fetch my profile from https://clawdecksearch.com/profile" (after login)
    ...
    async def show_cache_stats(
        ...
        if not self.website_cache:
            return ("ℹ️  Website caching is not enabled.\n\n"
                   "To enable caching, add to your ~/.clawdeck/config.yaml:\n"
                   "```yaml\n"
                   "browser_use:\n"
                   "  cache:\n"
                   "    enabled: true\n"
    ...
    async def login_to_website(
        ...
        Args:
            url: Login page URL (e.g., "https://clawdecksearch.com/login")
        ...
        Examples:
            - "Login to https://clawdecksearch.com with user@example.com"
