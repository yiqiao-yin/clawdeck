"""
Clawdeck Plugin System - User-installable plugins.

Plugins are Python packages or single-file modules that extend Clawdeck
with new tools, skills, or hooks. They are installed in ~/.clawdeck/plugins/.
...
class PluginManager:
    ...
    def __init__(self, plugins_dir: Optional[Path] = None):
        self.plugins_dir = plugins_dir or (Path.home() / ".clawdeck" / "plugins")
    ...
    def install_from_directory(self, source_path: str, name: Optional[str] = None) -> bool:
        ...
        Copies the directory into ~/.clawdeck/plugins/<name>/.
        """
    ...
    def create_plugin(self, name: str, plugin_type: str = "tool",
        ...
        manifest = {
            "name": name,
            "version": "0.1.0",
            "description": description or f"Clawdeck {plugin_type} plugin",
...
from clawdeck.hooks import HookPoint, HookContext, HookResult
    ...
    def _load_plugin(self, manifest: PluginManifest) -> LoadedPlugin:
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"clawdeck_plugin_{manifest.name}", str(entry)
            )
