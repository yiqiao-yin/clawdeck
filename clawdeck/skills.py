"""
Clawdeck Skills System - User-extensible slash commands via YAML files.

Skills are loaded from:
  1. ~/.clawdeck/skills/  (user-level)
  2. .clawdeck/skills/    (project-level, higher priority)
...
class SkillRegistry:
    ...
    def __init__(self,
        ...
        self.user_skills_dir = user_skills_dir or (Path.home() / ".clawdeck" / "skills")
        self.project_skills_dir = project_skills_dir or (Path.cwd() / ".clawdeck" / "skills")
...
# Place .yaml files in ~/.clawdeck/skills/ (user) or .clawdeck/skills/ (project)
name: review
